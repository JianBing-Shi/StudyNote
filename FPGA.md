...
FIFO *（First Input First Output）*，先入先出队列，一般指的是对数据的存储既有先进先出特性的一个缓存器，常被用于数据流处理、通信接口、时钟域跨越等场景，可以有效地解决数据传输中的**时序问题和数据丢失问题**。FIFO通常由一个存储器阵列和两个指针（读指针和写指针）组成，通过控制指针的移动来实现数据的存储和读取。FIFO相比较于其他存储资源没有“地址”的概念，因此设计简单，访问速度快，适合于高速数据传输和处理的场景。
### Flash Memory（闪存）
...
### 存储IP核（Memory IP Core）
* Block Memory Generator：块内存生成器，提供了一个图形化界面，帮助设计人员配置和生成BRAM的存储IP核。输入参数包括数据宽度、地址宽度、深度、读写模式等，输出一个可用于FPGA设计的存储IP核。
* FIFO Generator：FIFO生成器，提供了一个图形化界面，帮助设计人员配置和生成FIFO的存储IP核。输入参数包括数据宽度、地址宽度、深度、读写模式等，输出一个可用于FPGA设计的FIFO IP核。
* Memory Interface Generator：存储接口生成器，提供了一个图形化界面，帮助设计人员配置和生成与外部存储器（如DDR、SDRAM等）接口的存储IP核。输入参数包括存储器类型、数据宽度、地址宽度、时序参数等，输出一个可用于FPGA设计的存储接口IP核。