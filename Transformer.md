...
该模型的输入和输出遵循**相同尺寸**的原则，因此能无限层叠。Transformer 子层通常保持特征形状，任务 head 可以改变输出形式。
...
为了丰富网络参数，增强编解码能力。
将输入矩阵拆成N份Head，每份Head过一个Attention模组，最后将输出矩阵N份合成一份。每个 head 可能学习不同的关系。（模仿**随机森林**）

## Transformer Encoder
把输入加工成上下文表示，例如通过上下文帮助判断代词 *it*，*他她它*等指代谁。将原始输入转换为包含丰富上下文关系的特征表示。

* input： 	一段序列信息
* output：	带有融合了整个输入序列上下文的序列信息
### Encoder中的Self-Attention
Self-Attention是Attention的一种特殊形式，核心是“查询、键、值来自同一输入”，即模型对自身输入序列内部的元素计算相关性。

## Transformer Decoder
根据已有输出和Encoder表示，逐步生成结果。

* input：	目标序列的已知前缀，*训练时通常是右移后的目标序列*
* output：	目标序列的已知前缀再输出后一个序列
